...
void SCDF_Init(char* ip_local,char* ip_remote,int port_id,int* SOCK_sd,struct sockaddr_in *localport_info)
{
	...
	static unsigned int iOptVal_pkt;
	static unsigned int ActualOptVal_pkt;
	static socklen_t iOptLen_pkt = sizeof(ActualOptVal_pkt);
	unsigned int rcvTimeOut_pkt = 10;
...
//	if( rcvTimeOut_pkt>0 )
//	{
//		if (setsockopt(*SOCK_sd, SOL_SOCKET, SO_RCVTIMEO, (char *) &rcvTimeOut_pkt, sizeof(rcvTimeOut_pkt))==-1)
//		{
//		    printf("SCDF-> Error on setsockopt() timeout %d\n",errno);
//		}
//	}
	...
	if(iOptVal_pkt != ActualOptVal_pkt)
	{
		printf("SCDF-> Warning on port %d at getsockopt(): cannot fit the requested size buffer! desired %u actual %u\n",port_id,iOptVal_pkt,ActualOptVal_pkt);
	}
