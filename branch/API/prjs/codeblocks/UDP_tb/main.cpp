...
AppleMidiCh_ConfigUDPCtrlChReceiver(p_AppleMidiCtrlChannel,LocalIp,receiver_dst_ctrl);
...
/**** WAIT FOR CTRL CHANNEL TO SEE INVITATOR IP AND CONFIGURE SENDERS*******/
while (init_flag)
{
 tv  = {10,0};
result = select(maxfd+1, &readset,NULL , NULL, &tv);//&writeset
 if (FD_ISSET(my_read_fds[0], &readset))
 {
            printf("CTRL CHANNELL RECEIVED\n");
          //  AppleMidiCh_ParsePacket(p_AppleMidiCtrlChannel);
            AppleMidiCh_ParsePacketAndGetRemoteIp(p_AppleMidiCtrlChannel);
            ...
            if (ctrl_ch_state==Invitation)
            {
             ...
             AppleMidiCh_ConfigUDPSender(p_AppleMidiCtrlChannel,LocalIp,p_AppleMidiCtrlChannel->Invitator_ip, p_AppleMidiCtrlChannel->Invitator_port, sender_src_ctrl);
             AppleMidiCh_SendPkt(p_AppleMidiCtrlChannel, ctrl_tx_pkt_len);
             /** After found remote ip cpnfigure stream channel */
             AppleMidiCh_Init(&p_AppleMidiStreamChannel);
            AppleMidiCh_ConfigUDPReceiver(p_AppleMidiStreamChannel,LocalIp,RemoteIp, receiver_dst_stream, receiver_srstream);
            AppleMidiCh_ConfigUDPSender(p_AppleMidiStreamChannel,LocalIp,RemoteIp, sender_dst_stream, sender_src_stream);
            AppleMidiCh_GetSocketDescriptors(p_AppleMidiStreamChannel,  &my_write_fds[1],&my_read_fds[1]);
