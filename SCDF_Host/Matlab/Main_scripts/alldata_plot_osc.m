...
%         str_light_len = [' Frame_len = ',num2str(light_len)];
    ...
    if mod(k,refresh_rate)==0
        ...
        subplot(4,5,1)
        plot(t_axe,audio_data_plot);
        title(' AUDIO ');
        ylim([-1 1]);
%         text(5,0.8,str_audio_len);
        subplot(4,5,2)
        plot(t_axe,accel_x_data_plot);
        title(' ACCEL X ');
        ylim([-5 5]);
        subplot(4,5,3)
        plot(t_axe,accel_y_data_plot);
        title(' ACCEL Y ');
        ylim([-5 5]);
        subplot(4,5,4)
        plot(t_axe,accel_z_data_plot);
        title(' ACCEL Z ');
        ylim([-5 5]);
        subplot(4,5,5)
        plot(t_axe,proxy_data_plot);
        title(' PROXIMITY ');
        ylim([-0.5 5.5]);
        subplot(4,5,6)
        plot(t_axe,light_data_plot);
        title(' LIGHT ');
        subplot(4,5,7)
        plot(t_axe,udp_buff_plot);
        title(' UDP BUFFER ');
        subplot(4,5,8)
        plot(t_axe,gyro_x_data_plot);
        title(' GYRO X ');
       ylim([-5 5]);
        subplot(4,5,9)
        plot(t_axe,gyro_y_data_plot);
        title(' GYRO Y ');
        ylim([-5 5]);
        subplot(4,5,10)
        plot(t_axe,gyro_z_data_plot);
        title(' GYRO Z ');
        ylim([-5 5]);
        subplot(4,5,11)
        plot(t_axe,magneto_x_data_plot);
        title(' magneto X ');
         ylim([-100 100]);
        subplot(4,5,12)
        plot(t_axe,magneto_y_data_plot);
        title(' magneto Y ');
         ylim([-100 100]);
        subplot(4,5,13)
        plot(t_axe,magneto_z_data_plot);
        title(' magneto Z ');
         ylim([-100 100]);
         subplot(4,5,14)
        plot(t_axe,n_accel_data_plot);
        title(' n data Accel ');
        subplot(4,5,15)
        plot(t_axe,n_gyro_data_plot);
        title(' n data gyro ');
        subplot(4,5,16)
        plot(t_axe,n_magneto_data_plot);
        title(' n data magneto ');
         subplot(4,5,17)
        plot(t_axe,audio_frame_time_data_plot);
        title(' frame time stamp ');
         subplot(4,5,18)
        plot(t_axe,audio_timeid_data_plot);
        title('time ID');
%         ylim([0 30]);
...
if k==kmax
    ...
    k=1;
    %         i=1;
%              wavwrite(audio_data_plot,Fs,'test_audio.wav');
    audio_data_plot=zeros(1,len_plot);
    audio_frame_time_data_plot=zeros(1,len_plot);
    audio_timeid_data_plot=zeros(1,len_plot);
    accel_x_data_plot=zeros(1,len_plot);
