...
% Fs = 22050;
samples_mono = samples((1:fix(end/2)),1);
...
% 
    [events_got,events_got_back,event_lengts] = extract_events2(actual_frame,old_frame,ind(1:num),time_left,time_right,Fs ,max_time_right); 
% 
...
%     Hd_dc = highpass_iir(AudioInfo.SampleRate,10,200,60,1)http://www.libero.it/?cs_referrer=posta.libero.it;
      ...
      clean_event_spec=denoiser (events_got,events_got_back,len_fft,num);

     [spect,percent_pow,band_peak,vara,balance]= snap_recognition2(clean_event_spec,num,Fs,len_fft,freq_left,freq_right,max_events_per_frame);
